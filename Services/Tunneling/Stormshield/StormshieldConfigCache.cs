...
internal sealed class StormshieldConfigCache : IStormshieldConfigCache
{
        ...
        Guid tunnelConfigId, StormshieldSettings settings, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Schema gate. v1/v2 cache records may have had compression/framing directives stripped before
            // being written; that information is unrecoverable, so older records must miss and re-download.
            if (record.SchemaVersion != StormshieldCacheRecord.CurrentSchemaVersion)
            {
                _logger.LogDebug(
                    "Stormshield config cache for {TunnelId} has unsupported schema v{Version}; treating as a miss.",
    ...
    // DPAPI-encrypts and atomically writes an already-built record verbatim.
