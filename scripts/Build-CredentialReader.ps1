...
$sourceDir = Join-Path $repoRoot 'tools\wormhole-credential-reader'
$stagingDir = Join-Path $repoRoot 'dist-electron'
$binaryPath = Join-Path $stagingDir "wormhole-credential-reader-$Arch.exe"
...
if (-not $go) {
    throw 'Go is required to build the Windows credential reader.'
}
...
try {
    ...
    $pushed = $true
    try {
        Write-Info "BUILD wormhole-credential-reader.exe ($Arch)"
