...
| SSH | `wormhole-ssh` password path; SOCKS5 transport when the lease exposes a SOCKS endpoint (required for SSH-over-tunnel); host-key **verify** stub `verify_ssh_host_key` (Accept / Reject / Prompt; Fake store; LabOnly) + gate stub `gate_ssh_host_key` / `FakeKnownHosts` (prompt Accept-pin / Reject fail-closed; no live SSH in unit tests; orchestrator still uses `ssh_accept_any_host_key` until UI prompt is wired) |
