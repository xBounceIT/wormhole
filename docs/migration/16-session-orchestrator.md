...
**Status:** skeleton crate — Serial / SSH (password) / HTTP target dispatch + optional tunnel lease; RDP/VNC typed stubs; SSH Fake reconnect glue; connection progress stepper Fake glue; tunnel route prompt Fake glue; UI callers prepare via tree Open / Quick Connect glue
...
| Cancel | `tokio_util::sync::CancellationToken` on `ConnectOptions` (checked before/during tunnel + protocol); cancel lands `Failed` + `Cancelled`, never leaves `Connecting`. Tab close glue cancels the same token via [`close_tab_and_dispose_session`](../../rust/crates/wormhole-app/src/session_tabs.rs). Connecting overlay progress: [`FakeConnectionProgressGlue`](../../rust/crates/wormhole-session/src/connection_progress.rs) mirrors C# `ConnectionProgress` / `ConnectionProgressView` (Resolve/Tunnel/Auth/Connect lab phases; C# tunneled production shows Tunnel+Connect only; cancel mid-flight → reset fail-closed; tunnel sub-status via `describe_tunnel_phase`; no secrets on `Debug`; GPUI overlay Pending) |
| Route prompt | Per-connect tunnel-vs-direct when `PromptBeforeTunnelConnect` + `tunnel_enabled`: [`resolve_tunnel_route`](../../rust/crates/wormhole-session/src/tunnel_route_prompt.rs) / [`FakeTunnelRoutePromptUi`](../../rust/crates/wormhole-session/src/tunnel_route_prompt.rs) (AllowTunnel / PreferDirect / Cancel; Cancel fail-closed; setting off auto-routes; **not** wired into `connect` yet) |
