...
| Session approval | `SessionApprovalGate` approve/deny channel (auto-deny by default; tests can auto-approve or drive the channel) |
| Surface | `GET /health` + MCP tools/list matching C# `McpSshTools` names: `list_sessions`, `run_command`, `send_text`, `read_terminal` (call stubs; SSH registry later) |
...
cargo check -p wormhole-mcp --no-default-features   # placeholder + token/approval/capability helpers
```

Tests cover: real bind + health, reject bad/missing token, port `0` rejected, non-loopback / hostile bind strings rejected (unit + integration), tools list names, approval channel, capability glue (Fake report + off-loopback fail-closed + Debug redaction).
...
- Wiring live SSH tabs into `McpSshTools` (tool **names** are advertised via tools/list + capability Fake; session registry / command execution remain TODO — `FakeMcpCapabilityServer::execute_tool` fail-closed).
