...
Tests cover: real bind + health, reject bad/missing token, port `0` rejected, non-loopback / hostile bind strings rejected (unit + integration), tools list names, approval channel, capability glue (Fake report + off-loopback fail-closed + Debug redaction), session-registry Fake (Connected-only register, duplicate/unknown fail-closed, list order, Debug redaction).
...
- Wiring live SSH tabs / Fake registry into Streamable HTTP `McpSshTools` dispatch (tool **names** + `FakeMcpSessionRegistry` list/register glue exist; HTTP `list_sessions` still returns `[]` and `run_command` / `send_text` / `read_terminal` stay unwired — `FakeMcpCapabilityServer::execute_tool` fail-closed).
