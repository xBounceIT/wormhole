...
| PBKDF2 PIN/password verify | Spike in `wormhole-secrets-win::app_auth_service` — `AppAuthenticationService` set/verify/clear; Fake protector for unit tests (no live DPAPI); modes Disabled/Pin/Password (+ WindowsHello fallback slot only); wrong/missing/corrupt / oversized `Iterations` → fail closed; ASCII PIN + UTF-16 password length; Debug never echoes secrets ([adversarial-ledger-app-auth-pin.md](adversarial-ledger-app-auth-pin.md)). Interactive lock UI still Pending. |
| Idle-lock timeout policy | Spike in `wormhole-secrets-win::idle_lock` — `AppIdleLockGlue` + `FakeIdleClock` (last-activity; Disabled/Never never lock; zero/negative minutes fail-closed; no GPUI / no `GetLastInputInfo`) ([adversarial-ledger-idle-lock.md](adversarial-ledger-idle-lock.md)). Host timer + OS idle sampling Pending. |
