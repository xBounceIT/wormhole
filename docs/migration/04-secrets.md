...
| `app-auth.dpapi` | UTF-8 `Wormhole.AppAuthentication.v1` | Path + entropy + stub unlock (`app_auth`) + PIN/password set/verify/clear (`app_auth_service`); idle-lock policy (`idle_lock` / Fake clock); Hello availability / WinRT gap in `hello` |
...
### Credential password resolution (Lab Fake glue)

C# `CredentialPasswordResolver` / `SshCredentialResolver` password path resolves saved
credential bodies at connect time. Rust lab glue:

| Piece | Location |
|---|---|
| Orchestrator | `credential_password_resolver` — `CredentialPasswordResolverGlue` + `CredentialPasswordResolver` trait |
| Local passwords | `FakePasswordStore` / `PasswordStore` (`Wormhole:<guid>`) |
| Bitwarden passwords | `FakeBitwardenVaultPasswords` keyed by item id (no `bw` JSON) |
| Session gate | `BitwardenSession` — locked → `VaultLocked`; vault disabled → `VaultDisabled` |
| Virtual ids | Compose `BitwardenCredentialCatalogGlue::get_by_id` then `read_password` |
| Fail-closed | Empty / whitespace password, missing item ref, missing item, unsupported field path, SSH-key kind |

**No** live `bw` spawn, **no** unlock prompt callback (session must already be unlocked in Fake).
`Debug` / errors never embed password strings — use `resolved_password_len` for length-only diagnostics.

See [adversarial-ledger-credential-resolve.md](adversarial-ledger-credential-resolve.md).
...
**Idle-lock timeout glue** (`idle_lock`): `AppIdleLockGlue` + `FakeIdleClock` tracks
last-activity (C# `AppInactivityLockEvaluator` / `MarkUnlocked` spirit).
`Disabled` and UI `None` ("Never") never lock; zero/negative minutes **fail closed**
(lock when auth enabled); activity / unlock resets the idle window. No GPUI /
`GetLastInputInfo` — hosts supply ticks + OS idle later. See
[adversarial-ledger-idle-lock.md](adversarial-ledger-idle-lock.md) and
