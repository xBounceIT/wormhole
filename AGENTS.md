...
connection manager (SSH, RDP, VNC, SFTP file transfer, HTTP/HTTPS) positioned
...
- SFTP is not a standalone session protocol — `ProtocolType` is `Ssh`/`Rdp`/`Http`/`Https`/`Vnc`. SFTP file transfer is available from connected SSH tabs (see `SftpService` + the File Transfer dialog). Quick Connect and mRemoteNG import remain SSH/RDP/VNC-only (no HTTP/HTTPS yet).
