...
connection manager (SSH, RDP, HTTP/HTTPS, Serial, SFTP file transfer) positioned as a philosophical sequel to
...
- SFTP is not a standalone session protocol — `ProtocolType` is `Ssh`/`Rdp`/`Http`/`Https`/`Serial`. SFTP file transfer is available from connected SSH tabs (see `SftpService` + the File Transfer dialog). Quick Connect is SSH/RDP/Serial; mRemoteNG import remains SSH/RDP-only (no HTTP/HTTPS/Serial yet).
