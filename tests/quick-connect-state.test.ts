...
test('Quick Connect exposes VPN only for supported network surfaces', () => {
  const supported: QuickConnectProtocol[] = ['rdp', 'http', 'https', 'vnc'];
  const unsupported: QuickConnectProtocol[] = ['ssh', 'serial'];
...
test('Quick Connect maps the selected tunnel only when routing is supported and enabled', () => {
  ...
  assert.equal(quickConnectTunnelId('ssh', tunnelID), undefined);
