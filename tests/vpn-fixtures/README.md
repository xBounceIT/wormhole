...
WireGuard server, an OpenVPN server, and a TCP echo target reachable only through
either tunnel. The integration tests then build a real tunnel handshake against
each server and verify a 4-byte round-trip through the resulting SOCKS5 endpoint.
...
All three containers share the `vpn-test-net` bridge (10.20.0.0/24):
