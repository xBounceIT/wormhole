...
test('tunnel test IPC accepts an omitted probe or one complete bounded target', () => {
  assert.deepEqual(parseTunnelTestRequest({ id: tunnelID }), { id: tunnelID });
  assert.deepEqual(
    parseTunnelTestRequest({ id: tunnelID, targetHost: ' server.internal ', targetPort: 443 }),
    { id: tunnelID, targetHost: 'server.internal', targetPort: 443 },
  );
});
...
test('tunnel test IPC rejects malformed identifiers and partial or mistyped probes', () => {
  ...
  for (const request of [
    { id: 'not-an-id' },
    { id: tunnelID, targetHost: 42 },
    { id: tunnelID, targetHost: 'server.internal' },
    { id: tunnelID, targetPort: 443 },
    { id: tunnelID, targetHost: 'server.internal', targetPort: 0 },
    { id: tunnelID, targetHost: 'bad\nhost', targetPort: 443 },
    { id: tunnelID, targetHost: 'bad\u0080host', targetPort: 443 },
    { id: tunnelID, targetHost: 'bad host', targetPort: 443 },
    { id: tunnelID, targetHost: 'server.internal:443', targetPort: 443 },
    { id: tunnelID, targetHost: '[server.internal]', targetPort: 443 },
  ]) {
