...
func TestSSHTerminalEmulatorPublishesScrollbackRows(t *testing.T) {
	...
	if !strings.Contains(strings.Join(frame.Scrollback, "\n"), "one") {
	...
	if !changed || frame == nil || len(frame.Scrollback) != 1 || frame.Scrollback[0] != "three" {
...
func TestSSHTerminalEmulatorPreservesChunkedLargeOutput(t *testing.T) {
	...
	if frame.Scrollback[0] != "large-output-00997" || frame.Scrollback[len(frame.Scrollback)-1] != "large-output-05996" {
