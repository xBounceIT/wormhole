...
type State struct {
	...
	anydirty    bool
	cur         cursor
	curSaved    [2]cursor // DECSC storage belongs to each screen buffer.
...
func (t *State) saveCursor() {
	t.curSaved[t.screenIndex()] = t.cur
}

func (t *State) restoreCursor() {
	t.cur = t.curSaved[t.screenIndex()]
...
func (t *State) reset() {
	t.cur = t.defaultCursor()
	t.curSaved = [2]cursor{t.cur, t.cur}
	for i := range t.tabs {
		t.tabs[i] = false
	}
...
func (t *State) resize(cols, rows int) bool {
	...
	// Each viewport follows its own cursor. In particular, a low editor cursor
	// must not scroll the hidden shell away when the window becomes shorter.
	active := t.screenIndex()
	var slides [2]int
	slides[active] = max(0, t.cur.y-rows+1)
	slides[1-active] = max(0, t.curSaved[1-active].y-rows+1)
	if slide := slides[active]; slide > 0 {
		copy(t.lines, t.lines[slide:slide+rows])
	}
	if slide := slides[1-active]; slide > 0 {
		copy(t.altLines, t.altLines[slide:slide+rows])
	}
	for screen, slide := range slides {
		t.curSaved[screen].x = clamp(t.curSaved[screen].x, 0, cols-1)
		t.curSaved[screen].y = clamp(t.curSaved[screen].y-slide, 0, rows-1)
	}
	...
	if priv {
		for _, a := range args {
			...
			case 1049, // = 1047 and 1048
				...
				if a == 1049 && set {
					// Save before switching; a repeated entry saves on the alternate
					// screen and cannot overwrite the normal screen's cursor.
					t.saveCursor()
				}
				if alt && !set && a != 47 {
					t.clearAll()
