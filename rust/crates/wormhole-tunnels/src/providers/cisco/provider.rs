//! Cisco Secure Client — spawns `tools/wormhole-ciscoproxy` via [`SidecarProcess`].
//!
//! Wire protocol matches Fortinet: stdin JSON line, stdout `READY <port>`, stdin EOF = shutdown.
//! `secret_blob` is the sidecar config JSON (`CiscoSecureClientSidecarConfig` shape).
...
use super::super::secret_shape::require_cisco_establish_secret;
...
/// Cisco AnyConnect / Secure Client provider driving `wormhole-ciscoproxy.exe`.
///
/// Missing binary → [`TunnelError::BinaryNotFound`]. Never logs `secret_blob`.
pub struct CiscoSecureClientProvider {
...
impl CiscoSecureClientProvider {
    pub fn new() -> Self {
        Self {
    ...
    fn resolve_binary(&self) -> Result<PathBuf, TunnelError> {
        resolve_sidecar_binary(SidecarBinary::CiscoProxy, self.binary_override.as_deref())
    }
}

impl Default for CiscoSecureClientProvider {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl TunnelProvider for CiscoSecureClientProvider {
    ...
    async fn establish(
        ...
        require_cisco_establish_secret(secret_blob, &config.name)?;
...
mod tests {
    ...
    async fn missing_binary_returns_binary_not_found_not_up() {
        let provider = CiscoSecureClientProvider::with_binary_path(
            std::env::temp_dir().join("wormhole-ciscoproxy-missing-for-unit-test.exe"),
        );
        let err = match provider
            .establish(
                &TunnelConfigSnapshot::new(Uuid::nil(), TunnelKind::CiscoSecureClient, "lab"),
        ...
        assert!(
            rendered.contains("wormhole-ciscoproxy") || rendered.contains("not found"),
            "{rendered}"
        );
    }
    ...
    #[tokio::test]
    async fn rejects_pascal_case_editor_settings_before_locate() {
        let provider = CiscoSecureClientProvider::with_binary_path(
            std::env::temp_dir().join("wormhole-ciscoproxy-missing-shape-unit.exe"),
        );
        let err = match provider
            .establish(
                &TunnelConfigSnapshot::new(Uuid::nil(), TunnelKind::CiscoSecureClient, "lab"),
                br#"{"Host":"vpn.example","Password":"CISCO_SECRET_MARKER"}"#,
            )
            .await
        {
